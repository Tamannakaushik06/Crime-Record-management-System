...
global conn,cursor
conn = mysql.connector.connect(host='localhost',database='crime',user='root',password='12345')
cursor = conn.cursor()
...
def introduction():
    msg = '''
          ...
          Crime record are the most important part of any modern society for better controlling crime.

          The purpose of the project is to record the crime so that we can track the type of crime
          very easily and can also update the system.

          The project is divided into four major parts i.e addition of data, modification, searching and 
          reporting. 
...
def made_by():
    msg = ''' 
            Crime Record information system made by     :  Tamanna 
...
def login():
    while True:
        ...
        cursor.execute('select * from login where name="{}" and Passwod ="{}"'.format(uname,upass))
        cursor.fetchall()
...
def add_record():
    ...
    offence_id = int(input('Enter Offence_id : '))
